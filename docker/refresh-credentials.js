...
async function getToken() {
  try {
    ...
    return newToken.data.token; // USER TO CHANGE BASED ON THEIR PAYLOAD
  } catch (e) {
    throw e;
  }
}
...
async function updateSecretManager(token) {
  try {
    ...
    const patchRes = await k8sApi.patchNamespacedSecret(
      'my_secret',
      'default',
      patchPayload,
      undefined,
